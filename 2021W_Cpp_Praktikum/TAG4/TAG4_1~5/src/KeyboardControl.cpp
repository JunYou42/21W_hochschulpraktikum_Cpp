...
KeyboardControl::KeyboardControl():m_setVel({0.0,0.0}),m_getVel({0.0,0.0}),m_iMicros({1500,1500})
{
...
void KeyboardControl::Communicate()
{
	...
	while(1)
	{
		...
		else if(iNumber != -1)
		{
			...
			case 100:  p_setVel[0] -= 0.005; p_setVel[1] += 0.005; break;// d turn left
			Default:  break;
			}
...
//	cout << "Target left velovity(m/s): " << m_setVel[0] << ". Target right velovity(m/s): " << m_setVel[1] << endl;
	cout <<  m_cBuffer << endl;
...
/* jianhua!???*/
	int m_Signal[2];
	m_Signal[0] = m_setVel[0]*1000 + 1500;
	m_Signal[1] = m_setVel[1]*1000 + 1500;
